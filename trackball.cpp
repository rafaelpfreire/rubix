...
TrackBall::TrackBall() : RAD2DEG(180.0/3.141517)
{
    velocity = 0.0;
    trackingMouse = false;
...
QQuaternion TrackBall::getRotation(){
    if (trackingMouse) return rotation;
...
void TrackBall::mouseMove(const QPointF &p){
    ...
    if (msecs){
        ...
        (lenSqr >= 1.0) ? currentPos3D.normalize() : currentPos3D.setZ(sqrt(1 - lenSqr));
        axis = QVector3D::crossProduct(lastPos3D, currentPos3D);
        double angle = RAD2DEG * axis.length() * 0.005;
...
void TrackBall::mousePress(const QPointF &p){
    ...
    (lenSqr >= 1.0) ? lastPos3D.normalize() : lastPos3D.setZ(sqrt(1 - lenSqr));
...
QVector3D TrackBall::mousePosTo3D(const QPointF &p){
    return QVector3D(2.0 * p.x() / viewPortWidth - 1, 1.0 - 2.0 * p.y() / viewPortHeight,
                     0.0);
}
